...
def process_single_simulation(file_path, intervals, tree, window, seed):
    ...
    one element of the null distribution.
    """
    # Set unique random seed for this process
    # np.random.seed(seed)
    # np.random.seed(seed)
    
    # Initialize damage profile arrays
    rel_dmg_same = [0.0] * (2*window + 1)
    ...
    while data:
        ...
        if source[0] == "NEW":
            ...
            if len(arr) > 0:
                ...
                total_this_4mer = sum(x[3] for x in redistributed)
                # print(f"Process {os.getpid()} - 4mer {current_id}: Expected {index[current_id]}, Got {total_this_4mer}")
                ...
                for damage in redistributed:
                    if damage[3] > 0:  # if there is damage at this position
                        for match in overlap(damage, tree):
                            ...
                            if abs(dist) <= window:
                                dmg = match[2]
                                if match[3]:  # same strand
                                    rel_dmg_same[dist + window] += dmg
                                else:  # different strand
                                    rel_dmg_diff[dist + window] += dmg
            
            if source[1] == '000':  # Changed to string comparison
                break
    ...
    # print(f"Process {os.getpid()}: Total redistributed damages: {total_redistributed}")
    # print(f"Process {os.getpid()}: Sum of same strand: {sum(rel_dmg_same)}, diff strand: {sum(rel_dmg_diff)}")
    
    return rel_dmg_same + rel_dmg_diff

def perform_single_simulation(intervals, tree, radius, file_path, run_id):
    """
    Wrapper function to match the interface expected by build_runs
    """
    return process_single_simulation(file_path, intervals, tree, radius, run_id)
...
def build_runs(num_runs, range_map, tf_tree, window):
    ...
    df[['min', 'max', 'median', 'mean']] = df.apply(
